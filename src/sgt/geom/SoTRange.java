...
public abstract class SoTRange implements java.io.Serializable, Cloneable {
   ...
   * <code>GeoDate</code>.
   * @since sgt 2.0
   * @deprecated As of sgt 3.0, replaced by {@link sgt.geom.SoTRange.Time SoTRange.Time}
   */
  public static class GeoDate extends SoTRange {
    /** The range's first time  */
    public sgt.geom.GeoDate start = null;
    /** The range's last time  */
    public sgt.geom.GeoDate end = null;
    /** The range's time increment  */
    public sgt.geom.GeoDate delta = null;
    /**
     ...
     */
    public GeoDate(sgt.geom.GeoDate tstart,
       sgt.geom.GeoDate tend) {
    ...
    public GeoDate(TimeRange trange) {
      start = new sgt.geom.GeoDate(trange.start.getTime());
      end = new sgt.geom.GeoDate(trange.end.getTime());
      if(trange.delta != null) {
        delta = new sgt.geom.GeoDate(trange.delta.getTime());
      } else {
        delta = null;
      }
    }
    ...
    public SoTRange copy() {
      sgt.geom.GeoDate cStart = null;
      sgt.geom.GeoDate cEnd = null;
      sgt.geom.GeoDate cDelta = null;
      if(start != null)
        cStart = new sgt.geom.GeoDate(start.getTime());
      if(end != null)
        cEnd = new sgt.geom.GeoDate(end.getTime());
      if(delta != null)
        cDelta = new sgt.geom.GeoDate(delta.getTime());
  ...
  public static class Time extends SoTRange {
    ...
    public Time(sgt.geom.GeoDate ustart,
                sgt.geom.GeoDate uend) {
